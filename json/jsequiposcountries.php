...
$cad = '';
	while ($row = mysql_fetch_array($resTraerDatos)) {
		...
		if ($datos == 1) {
			...
			array_push($ar,array('equipo'=>$row[0].' - '.$row['nombre'], 'id'=> $row[0], 'iddivision'=>$row['refdivisiones']));
		} else {

			array_push($ar,array('equipo'=>$row['nombre'].' - '.$row['categoria'].' - '.$row['division'], 'id'=> $row[0], 'iddivision'=>$row['refdivisiones']));
		}
	}
