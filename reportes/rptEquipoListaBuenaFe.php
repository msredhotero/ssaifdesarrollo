...
if (mysql_num_rows($resEquipoAux) <=0) {
   $resEquipo =  $serviciosReferencias->traerEquiposPorEquipo($refEquipos);
   $resEquipoAux =  $serviciosReferencias->traerEquiposPorEquipo($refEquipos);
} 



$resDatos = $serviciosReferencias->traerConectorActivosPorEquipos($refEquipos);
...
while ($rowE = mysql_fetch_array($resDatos)) {
   ...
   $existeExcepciion = mysql_num_rows($serviciosReferencias->traerExcepcionPorJugadorEquipoTemporada($rowE['refjugadores'],$refEquipos,$reftemporadas));
