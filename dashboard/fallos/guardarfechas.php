...
$id		=	$_POST['id'];
	...
	for($i=0;$i<$numero;$i++){
		...
		/////////////////////			EQUIPO LOCAL				////////////////////////////////////////////////
		if (strpos($tags[$i],"fecha") !== false) {

			$mod = explode('fecha',$tags[$i]);
			$idSancionJugador = $mod[0];
			$idFechas = $mod[1];
			$cumplidas = $mod[2];
			$orden = $mod[3];
			
			if ($cambio != $idSancionJugador) {
				//primero borro todas las fechas de los movimientos
				$serviciosReferencias->eliminarMovimientosancionesPorSancionJugador($idSancionJugador);
				$cambio = $idSancionJugador;
			}
			
			//verificar que no haya mas fechas de lo fallado
			$res = $serviciosReferencias->insertarMovimientosancionesManual($idSancionJugador, $idFechas,$cumplidas, $orden);
