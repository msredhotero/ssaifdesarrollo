...
function calcularPuntoBonus($refTorneo, $idEquipo) {
    ...
    //determinar ultima fecha jugado del torneo
    $ultimaFecha    =   $this->traerUltimaFechaFixturePorTorneoEquipoNuevo($refTorneo, $idEquipo);
...
, (SELECT @rownum:=0) R ";
    ...
    if ($ultimaFecha == 1) {
        $sorted = $this->array_orderby($arPosiciones,'pj', SORT_DESC, 'puntos', SORT_DESC, 'rojas', SORT_ASC, 'amarillas', SORT_ASC, 'goles', SORT_DESC, 'golescontra', SORT_ASC);
    } else {
        $sorted = $this->array_orderby($arPosiciones, 'puntos', SORT_DESC, 'rojas', SORT_ASC, 'amarillas', SORT_ASC, 'goles', SORT_DESC, 'golescontra', SORT_ASC);
    }
...
function traerProximaFechaTodosReal($desde, $hasta) {
    ...
    $sql = "select
        ...
        order by cat.orden, tor.refdivisiones, f.idfecha, fix.idfixture
        ";
...
function traerHistoricoIncidenciasPorJugador($idJugador, $where) {
    $sql = "SELECT
            ...
            FROM
                ...
                WHERE
                    jug.idjugador = ".$idJugador.$where." /*and mij.minutos > 0*/
...
function traerEstadisticaJugadorPorCategoria($where, $whereAux) {
    $sql = "SELECT
            ...
            FROM (
                ...
                WHERE
                    es.finalizado = 1 and p.minutos > 0 ".$where."
                ...
                INNER JOIN tbtemporadas tep ON tep.idtemporadas = tor.reftemporadas
                WHERE
                    p.reftiposanciones IN (2 , 3, 4, 5) ".$where."
                        AND sf.amarillas=2
                group by jug.apellido, jug.nombres,
                        jug.nrodocumento,
                        p.refjugadores,
    ...
    function rotarImagen($imagen, $direccion, $directorio) {
        ...
        if (($pos !== false) || ($pos2 !== false)) {
           ...
           //die(var_dump($directorio));
            ...
            $image_rotate = "./../../data/3658".'/imagen_rotate.jpg';
...
$sql = "SELECT
            ...
            j.reftipodocumentos,
            j.refcountries,
            coalesce( CONCAT('data',
                    '/',
                    di.iddocumentacionjugadorimagen,
                    '/',
                    di.imagen), '') AS imagen
        FROM
            dbjugadores j
            ...
            tbposiciontributaria po ON po.idposiciontributaria = cou.refposiciontributaria
                LEFT JOIN
            dbdocumentacionjugadorimagenes di ON di.idjugador = j.idjugador
            ...
            FROM
                ...
                GROUP BY go.reffixture , jug.idjugador UNION ALL SELECT
                    0 AS goles,
                        0 AS encontra,
                        0 AS penal,
                        SUM(go.cantidad) AS amarillas,
                        0 AS rojas,
                        0 AS pj,
                        go.reffixture,
                        jug.idjugador
                FROM
                    dbsancionesjugadores go
                INNER JOIN dbjugadores jug ON jug.idjugador = go.refjugadores
                ...
                UNION ALL SELECT
                    0 AS goles,
                        ...
                        0 AS amarillas,
                        0 AS rojas,
                        coalesce(COUNT(go.idminutojugado),0) AS pj,
                        go.reffixture,
                        jug.idjugador
                FROM
                    dbminutosjugados go
        ...
        $sql = "
            ...
            from (
                ...
                SELECT
                    ...
                    CONCAT(je.apellido, ' ', je.nombres) AS apynentra,
                    fix.nombreequipolocal AS equipo,
                    1 AS orden
                FROM
                    dbcambios c
...
FROM
    ...
    dbdorsales ds ON c.refdorsalsale = ds.numero
        ...
        AND fix.refconectorvisitante = ds.refequipos
        LEFT JOIN
    dbjugadores js ON js.idjugador = ds.refjugadores
        INNER JOIN
    dbtorneos t ON t.idtorneo = fix.reftorneos
